...
import { useOrgKpisOverview, useAtRiskStudents } from "@/lib/hooks/use-org-kpis";
import { useCohortsKpis } from "@/lib/hooks/use-org-kpis";

export default function OrgDashboardPage() {
  const { slug } = useParams<{ slug: string }>();
  const { data: kpis, isLoading } = useOrgKpisOverview(slug);
  const { data: cohortsKpis, isLoading: cohortsLoading } = useCohortsKpis(slug);
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Engagement and performance across your organisation.</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard
          icon={Activity}
          label="Active today"
          value={isLoading ? null : (kpis?.dau ?? 0)}
          color="text-accent"
        />
        <KpiCard
          icon={Users}
          label="Active this week"
          value={isLoading ? null : (kpis?.wau ?? 0)}
          color="text-teal-500"
        />
        <KpiCard
          icon={TrendingUp}
      ...
      {/* Per-cohort KPIs */}
      <section>
        <h2 className="font-sora text-lg font-semibold text-primary mb-3">Cohort breakdown</h2>
        {cohortsLoading ? (
        ...
        ) : (cohortsKpis ?? []).length === 0 ? (
          <p className="text-sm text-slate-400">No cohort data yet.</p>
        ) : (
          <div className="rounded-xl border border-slate-200 divide-y divide-slate-100">
            {(cohortsKpis ?? []).map((c) => (
