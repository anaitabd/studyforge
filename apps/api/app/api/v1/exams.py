...
async def list_exams(
    ...
    page_filter = [Exam.group_id == group_id]
    if cursor_dt is not None:
        page_filter.append(
            or_(
                Exam.created_at < cursor_dt,
                and_(Exam.created_at == cursor_dt, Exam.id < cursor_id),
            )
        )
    ...
    next_cursor = encode_cursor(exams[-1].created_at, exams[-1].id) if len(exams) == pagination.limit else None
...
async def submit_exam(
    ...
    schedule_award_xp(current_user.id, "exam_complete")
    if score_pct is not None:
        if score_pct >= 0.6:
            schedule_award_xp(current_user.id, "exam_score_60")
        if score_pct >= 0.8:
            schedule_award_xp(current_user.id, "exam_score_80")
        if score_pct == 1.0:
            schedule_award_xp(current_user.id, "exam_perfect")
