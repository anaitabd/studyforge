...
Create `backend/app/models/permissions.py`:
```python
...
Create `backend/app/models/events.py` — `UserEvent` model (non-ORM, raw insert via `asyncpg` for performance).
...
Create `backend/app/models/organization.py` and `backend/app/schemas/organization.py`.
...
Create `backend/app/api/v1/organizations.py`. Mount at `/api/v1/org`.
...
Extend `backend/app/api/v1/teacher.py` (existing file).
...
Extend `backend/app/api/v1/me.py` (existing file).
...
In `backend/app/tasks/analytics_tasks.py`:
...
- Add Pydantic v2 schemas for every new model in `backend/app/schemas/`
...
2. Read `backend/app/models/` — all existing models
3. Read `backend/app/api/v1/` — all existing route files
4. Read `backend/app/services/` — all existing services
5. Read `backend/app/core/security.py` — existing auth patterns
